...
const EditorConfig = ({
      ...
      callbacks: {
        onExport: async (blobs: any) => {
          ...
          if (template?.printPreview) {
            ...
            window.open(
              `/menu/printpreview/${template.id}/${template.printPreview}`
            );
          } else {
                ...
                if (user) {
                  ...
                  downloadBlobFile(
                    ...
                    removeSpecialCharacters(
                      menuContent?.name ?? template?.name
                    ) || ""
                  );
                } else {
                  openAuthDialog();
        ...
        onSave: (scene: any) => {
          ...
          setUserData((user: any) => {
            if (isAbleToUpdate) {
              ...
              if (user) {
                saveTemplate(scene);
                if (template?.printPreview) saveMenuToLibrary();
              } else {
                openAuthDialog();
              }
              setTimeout(() => {
        ...
        async (instance: any) => {
          ...
          if (scene && layout && router.pathname.includes("printpreview/")) {
            ...
            for (let i = 0; i < numPages - 1; i++) {
              const page = cesdkInstance.current.engine.block.duplicate(
                existingPages[0]
              );
    ...
    if (textContent !== "Dishes") {
      ...
      newElement.addEventListener("click", (e) => {
        ...
        InsertDishInCanvas({
          title: "dish",
          price: "$34",
          description: "this is a dish description",
        });
  ...
  const saveMenuToLibrary = async () => {
    ...
    async function groupComponents(page: string) {
      const childrenOfPage =
        cesdkInstance?.current.engine?.block?.getChildren(page);
      if (cesdkInstance?.current.engine?.block?.isGroupable(childrenOfPage)) {
        const group =
          cesdkInstance?.current?.engine.block.group(childrenOfPage);
        const savedBlock =
    ...
    try {
      ...
      const pageGroups = Promise.all(
        pageBlocks.map(async (page: string) => await groupComponents(page))
      );
  ...
  const injectMenuIntoCanvas = async () => {
    try {
      const menuPlaceholders =
        await cesdkInstance?.current?.engine?.block.findByName(
          "menu_placeholder"
        );
      ...
      for (let i = 0; i < pages.length; i++) {
        const pageChildren = cesdkInstance?.current.engine.block.getChildren(
          pages[i]
        );
        const placeholdersOnPage = pageChildren.filter((item: number) =>
          menuPlaceholders.includes(item)
        );
        console.log(
          "🚀 ~ file: EditorConfig.tsx:1161 ~ injectMenuIntoCanvas ~ placeholdersOnPage:",
          placeholdersOnPage
        );
        return;
        for (let n = 0; n < placeholdersOnPage.length; n++) {
          ...
          cesdkInstance?.current.engine.block.appendChild(
            placeholdersOnPage[n],
            block[0]
          );
          cesdkInstance?.current.engine.block.setPositionX(block[0], 0.05);
          cesdkInstance?.current.engine.block.setPositionY(block[0], 0);
          cesdkInstance?.current.engine.block.setFillEnabled(
            placeholdersOnPage[n],
            false
          );
          cesdkInstance?.current.engine.block.setStrokeEnabled(
            placeholdersOnPage[n],
            false
          );
        }
